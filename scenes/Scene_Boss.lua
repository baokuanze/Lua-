...
local Scene_Boss = class("Scene_Boss", function()
    return display.newScene("Scene_Boss")
end)

function Scene_Boss:ctor()
 ...
 self.id = scheduler.scheduleGlobal(handler(self, Scene_Boss.updatePos),1/60)  --- 遥感




end 
function  Scene_Boss:initHRocker()
   --添加摇杆
  print("--------------------------" .. type(HRocker))
...
function Scene_Boss:herorun()  --- 辅助英雄的轨迹  
    local speed = 100
...
function Scene_Boss:heroattack ()
      local speed = 100
    local mindis = 0
    local v1 ={}
...
function Scene_Boss:updateMons()
...
--     end            
-- end 
function Scene_Boss:Ms3_v3(v , i)
    -- local frame = self.hero.player:getTextureRect()
...
function Scene_Boss:Ms1_v1(v , i)
    -- local frame = self.hero.player:getTextureRect()
       local frame = self.hero.player:getBoundingBox()
       -- local frame = self.hero.player:getBone("weapons"):getBoundingBox()
     local HeroRect = self:newHeroRect(self.hero,frame)
...
function Scene_Boss:Ms2_v2(v , i)
...
function  Scene_Boss:possible_win()
 for i,v in ipairs(self.Ms1Array) do
    if    v.MsHp<=0  then  
    table.remove(self.Ms1Array,i)  
    table.remove(CSData.MonsterArray,i)
...
function Scene_Boss:Herolose_Hp ()    -- 英雄失血
...
function Scene_Boss:Defeated() --失败
    scheduler.unscheduleGlobal(self.id)         --关闭碰撞检测
    scheduler.unscheduleGlobal(self.hero.is)    --关闭摇杆
    scheduler.unscheduleGlobal(self.rocker.id)    --  关闭 技能缓冲定时器
    audio.stopAllSounds()
...
function Scene_Boss:stopMster() --停止所有怪物的行动
...
function Scene_Boss:newMonsterRect( v,width,height )     --  怪物框架  
...
function Scene_Boss:newHeroRect( v,frame )
...
function Scene_Boss:onEnter()

end

function Scene_Boss:onExit()
    scheduler.unscheduleGlobal(self.id)         --关闭碰撞检测
    scheduler.unscheduleGlobal(self.hero.is)    --关闭摇杆
    scheduler.unscheduleGlobal(self.rocker.id)    --  关闭 技能缓冲定时器
end
