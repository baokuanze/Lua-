...
local Scene_fighting = class("Scene_fighting", function()
    return display.newScene("Scene_fighting")
end)

function Scene_fighting:ctor()
 ...
 self.id = scheduler.scheduleGlobal(handler(self, Scene_fighting.updatePos),1/60)  --- 遥感




end 
function  Scene_fighting:initHRocker()
   --添加摇杆
  print("--------------------------" .. type(HRocker))
...
function Scene_fighting:initHero()
   SkillData .Data.P1 = 0
  SkillData .Data.P2 = 0
  SkillData .Data.P3 = 0
  SkillData .Data.P4 = 0
...
function Scene_fighting:herorun()  --- 辅助英雄的轨迹  
    local speed = 100
...
function Scene_fighting:heroattack ()
      local speed = 100
    local mindis = 0
    local v1 ={}
...
function Scene_fighting:initMs()
    --添加怪物
  ...
  CSData.MonsterArray={}
   ...
   -- self.Ms1num = CSData.SCENE[Sc][Ch].boy1
     self.Ms1num =math.random(1,5)

  -- self.Ms2num = CSData.SCENE[Sc][Ch].boy2
 self.Ms2num= math.random(1,5)
 -- self.Ms2num=1
  -- self.Ms3num = CSData.SCENE[Sc][Ch].boy3
  -- self.Ms3num = math.random(1,2)
 self.Ms3num = math.random(0,2)
      -- table.insert(CSData.MonsterArray,MsNode3)
    self:updateMons()
 
end
function Scene_fighting:updateMons()
...
function Scene_fighting:initBg()
 ...
 -- sceneData.selectData = sceneData.data[1][1]
  local a =  math.random(1,7)
      ...
      sceneData.selectData = sceneData.data[a][b]
   local bg =   cc.Sprite:create(sceneData.data[a][b].imag) 
...
function Scene_fighting:updatePos() 
...
function Scene_fighting:Ms3_v3(v , i)
...
function Scene_fighting:Ms1_v1(v , i)
    -- local frame = self.hero.player:getTextureRect()
       local frame = self.hero.player:getBoundingBox()
       -- local frame = self.hero.player:getBone("weapons"):getBoundingBox()
     local HeroRect = self:newHeroRect(self.hero,frame)
...
function Scene_fighting:Ms2_v2(v , i)
...
function Scene_fighting: Ms1coll() 
      -- local frame = self.hero.player:getTextureRect()
     -- local frame = self.hero.player:getBoundingBox()
     --  local HeroRect = self:newHeroRect( self.hero,frame )
        --第一种怪物的碰撞检测
...
function  Scene_fighting:possible_win()
 for i,v in ipairs(self.Ms1Array) do
    if    v.MsHp<=0  then  
    table.remove(self.Ms1Array,i)  
    table.remove(CSData.MonsterArray,i)
...
function Scene_fighting:modify()
...
function Scene_fighting:Herolose_Hp ()    -- 英雄失血
...
function Scene_fighting:Defeated() --失败
    scheduler.unscheduleGlobal(self.id)         --关闭碰撞检测
    scheduler.unscheduleGlobal(self.hero.is)    --关闭摇杆
    scheduler.unscheduleGlobal(self.rocker.id)    --  关闭 技能缓冲定时器
    audio.stopAllSounds()
...
local loseLayer = losewujing.new()
...
function Scene_fighting:stopMster() --停止所有怪物的行动
...
function Scene_fighting:newMonsterRect( v,width,height )     --  怪物框架  
...
function Scene_fighting:newHeroRect( v,frame )
...
function Scene_fighting:onEnter()

end

function Scene_fighting:onExit()
    scheduler.unscheduleGlobal(self.id)         --关闭碰撞检测
    scheduler.unscheduleGlobal(self.hero.is)    --关闭摇杆
    scheduler.unscheduleGlobal(self.rocker.id)    --  关闭 技能缓冲定时器
end

return Scene_fighting
...
-- local   ArrowPathBezier= require("app.scenes.ArrowPathBezier")
-- function Scene_fighting:ctor()

-- -- local test  = require("app.scenes.test")
-- -- -- dump(test)
-- --     local path = cc.FileUtils:getInstance():getWritablePath()    
-- --     local   file = io.open(path.."test.lua",'w')    -- 对Excel 文件进行 读取 写入
...
-- -- display.addSpriteFrames("res/show/test.plist", "res/show/test.pvr.ccz")

-- -- cc.SpriteFrameCache:getInstance():addSpriteFrameWithFile("res/show/texture.plist")
-- display.addSpriteFrames("res/show/jiami.plist", "res/show/jiami.pvr.ccz")
-- local sp = display.newSprite("#shop_star_button_2_alpha_mask.png"):pos(display.cx, display.cy):addTo(self)
-- display.addSpriteFrames("res/show/texture.plist", "res/show/texture.pvr.ccz")
-- self:createAnimation ("player" ,8,0.06)
  
 


-- playerbody =  cc.Sprite:createWithSpriteFrameName("playerbody.png");
-- self.player = cc.Sprite:create()
-- playerbody:setAnchorPoint(0.5,0.5)  
-- self.player:addChild(playerbody)
-- self.player:setPosition(cc.p (100,100))
-- self:addChild(self.player)

--   self.playerarrow = cc.Sprite:createWithSpriteFrameName("playerarrow.png")
--   print(self.player:getPositionX() , self.player:getPositionY())
--   self.playerarrow:setPosition(self.player:getPositionX()+20,self.player:getPositionY()-10)
--  -- self. playerarrow:setAnchorPoint(0.2,0.2)
--   self:addChild(self. playerarrow,-1)
...
--   self.currTouchpiont =  touch:getLocation()
--   print("-------".. self.currTouchpiont.x)
--  print("-------".. self.currTouchpiont.y)
...
-- self.l:registerScriptHandler(function(touch , event) 
--  self:createAndShootArrow(self.currTouchpiont);
-- end,cc.Handler.EVENT_TOUCH_ENDED)
-- d:addEventListenerWithSceneGraphPriority(self.l, self)


-- end 
-- function  Scene_fighting:createAnimation (  str , num ,delay )
--    -- local animFrames ={}
-- -- for i=1,num do
-- -- 	local str =  string.format(  "player"..i..".png")
-- -- 	print(str)
...
--  local frames = display.newFrames("player%d.png", 1, num)
-- local animation = display.newAnimation(frames, 0.5 / 8) -- 0.5 秒播放 8 桢
--    display.setAnimationCache("player",animation)
-- end



-- function Scene_fighting:createAndShootArrow(  currTouchPoint)
--   isRunAction = true;
--   local  animation= display.getAnimationCache("player")
--   print(animation)
--    local  animate = cc.Animate:create(animation)
--    local delaytime = cc.DelayTime:create(0.5)
--   local  animate = cc.Animate:create(animation)
 
--  local funcall1 = cc.CallFunc:create(function ()
--  self :shootArrow() end)
--   local funcall2 = cc.CallFunc:create(function ()
-- self:finishRunAction() end)

--   local seq = cc.Sequence:create (animate, funcall1,delayTime , funcall2)
--    playerbody:runAction(seq)


-- end 
 
-- function Scene_fighting:finishRunAction()

--  isRunAction = false
-- end  
-- function Scene_fighting:rotateArrow( touchPoint)
-- end  


-- function  Scene_fighting:shootArrow()
-- local hx ,hy  =self.playerarrow :getPosition()
-- --point= self:convertToWorldSpace(cc.p(self.playerarrow :getPosition()))
-- -- local  diff= hx+5;
-- print("hx -----" .. hx)
-- print("hy ---"  .. hy)
--   local   arrow= ArrowPathBezier.new("arrow1.png")
-- 	-- local arrow  = cc.Sprite:createWithSpriteFrameName("arrow1.png")
-- 	 arrow:setRotation(self.playerarrow:getRotation())
--   arrow :setPosition(hx, hy)

-- 	 self:addChild(arrow)


    
--     -- local action_1 = cc.BezierTo:create()

--      -- local action = cc.ArrowPathBezier:create(2, bezier);
--      --  arrow:runAction(action);
--     local x = (self.currTouchpiont.x -hx)*2+hx
--     local y = hy 
--     print("x" .. x )
--     print("y".. y)
--     local action_1 = cc.BezierTo:create(2,{cc.p(hx,hy),cc.p(self.currTouchpiont.x, self.currTouchpiont.y),cc.p(x,y)})
--     transition.execute(arrow, action_1, {onComplete = function()
--       --arrow:stopAndClear()
--       end
--       })

   
--    local x1 = self.currTouchpiont.x -hx
--   local y1 = self.currTouchpiont.y - hy
--   local angle = math.atan2(y1, x1)
  
--   local angle_roate = angle*180/3.14 
-- transition.rotateTo(self.playerarrow, {rotate =-angle_roate , time = 0.5})

-- self:draw()
     
-- end  
-- function Scene_fighting:draw()
--    local hx ,hy = self.playerarrow:getPosition()
--    local x = (self.currTouchpiont.x -hx)*2+hx
--     local y = hy 
--  if (not self.nodedraw)then 
--    self.nodedraw = cc.DrawNode:create()
--      self:addChild(self.nodedraw, -2);  
--   end 
--    self.nodedraw:clear()
--    self.nodedraw :drawQuadBezier(cc.p(self.playerarrow:getPosition()), self.currTouchpiont, cc.p(x,y),100,  cc.c4f(1.0, 0.0, 0.0, 1.0))
